...
public class TodoController {
    ...
    @PatchMapping("/unselected/{member-id}/{todo-id}")
    public ResponseEntity unseletedTodo(@PathVariable("member-id") Long memberId,
                                                       @PathVariable("todo-id") Long todoId){
        Todo updateTodo = todoService.unseletedTodo(todoId, memberId);
        TodoResponseDto response = mapper.todoToTodoResponseDto(updateTodo);
        long point = memberService.memberPoint(memberId);
