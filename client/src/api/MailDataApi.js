...
export const getAllMails = async memberId => {
  ...
  const res = await axios.get(mockUrl + '/mails' + `/${memberId}`);
  return res.data;
};

export const deleteMail = async (memberId, mailId) => {
  // const res = await axios.delete(serverUrl + `/mails/${memberId}/${mailId}`);
  const res = await axios.delete(mockUrl + `/mails/${memberId}/${mailId}`);
  return res.data;
};
...
export const readMail = async (memberId, mailId) => {
  ...
  const res = await axios.patch(mockUrl + `/mails/${memberId}/${mailId}`);
  return res.data;
};
